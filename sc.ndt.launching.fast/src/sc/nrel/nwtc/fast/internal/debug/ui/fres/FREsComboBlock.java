...
import sc.nrel.nwtc.fast.internal.debug.fres.FREsPreferencePage;
...
public class FREsComboBlock {
	 ...
	 */
	private FREDescriptor fDefaultDescriptor = null;
	
	/**
	 * Specific JRE descriptor or <code>null</code> if none.
	 */
	private FREDescriptor fSpecificDescriptor = null;
	...
	private static IStatus OK_STATUS = new Status(IStatus.OK, FREIDebugUIPlugin.getUniqueIdentifier(), 0, "", null); //$NON-NLS-1$
	 ...
	 */
	public FREsComboBlock(boolean defaultFirst) {
	...
	public void createControl(Composite ancestor) {
		...
		if (fTitle == null) {
			fTitle = FREMessages.JREsComboBlock_3; 
		}
	...
	private void createEEControls(Composite comp) {
		fEnvironmentsButton = SWTFactory.createRadioButton(comp, FREMessages.JREsComboBlock_4);
			...
			public void widgetSelected(SelectionEvent e) {
				if (fEnvironmentsButton.getSelection()) {
					...
					if (fEnvironments.isEmpty()) {
						setError(FREMessages.JREsComboBlock_5);
					} else {
						setStatus(OK_STATUS);
					}
					firePropertyChange();
		...
		fManageEnvironmentsButton = SWTFactory.createPushButton(comp, FREMessages.JREsComboBlock_14, null);
	...
	*/
	...
	private void createAlternateJREControls(Composite comp) {
		String text = FREMessages.JREsComboBlock_1;
		...
		fSpecificButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if (fSpecificButton.getSelection()) {
					...
					if (fVMs.isEmpty()) {
						setError(FREMessages.JREsComboBlock_0);
					} else {
						setStatus(OK_STATUS);
					}					
		...
		fManageButton = SWTFactory.createPushButton(comp, FREMessages.JREsComboBlock_2, null); 
		fManageButton.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				showPrefPage(FREsPreferencePage.ID); 
			}
		});
	 ...
	 */
	protected void setJREs(List<FASTStandin> jres) {
		fVMs.clear();
		fVMs.addAll(jres);
		// sort by name
		Collections.sort(fVMs, new Comparator<Object>() {
			public int compare(Object o1, Object o2) {
				IFASTInstall left = (IFASTInstall)o1;
				IFASTInstall right = (IFASTInstall)o2;
		...
		int i = 0;
		while (iter.hasNext()) {
			IFASTInstall vm = (IFASTInstall)iter.next();
			names[i] = vm.getName();
			i++;
		}
	 ...
	 */
	private void selectFRE(IFASTInstall vm) {
	 ...
	 */
	public IFASTInstall getFRE() {
		int index = fCombo.getSelectionIndex();
		if (index >= 0) {
			return (IFASTInstall)fVMs.get(index);
		}
		return null;
	}
	 ...
	 */
	//private void /*IExecutionEnvironment*/ getEnvironment() {
	...
	protected void fillWithWorkspaceProfiles() {
		/*fEnvironments.clear();
		IExecutionEnvironment[] environments = FASTRuntime.getExecutionEnvironmentsManager().getExecutionEnvironments();
		...
		while (iter.hasNext()) {
			...
			IPath path = FASTRuntime.newJREContainerPath(env);
			IFASTInstall install = FASTRuntime.getFASTInstall(path);
			if (install != null) {
				names[i] = NLS.bind(FREMessages.JREsComboBlock_15, new String[]{env.getId(), install.getName()});
			} else {
				names[i] = NLS.bind(FREMessages.JREsComboBlock_16, new String[]{env.getId()});
			}
			i++;
		}
	 ...
	 */
	public void setDefaultFREDescriptor(FREDescriptor descriptor) {
		fDefaultDescriptor = descriptor;
		setButtonTextFromDescriptor(fDefaultButton, descriptor);
	}
	
	private void setButtonTextFromDescriptor(Button button, FREDescriptor descriptor) {
		if (button != null) {
	 ...
	 */
	public void setSpecificFREDescriptor(FREDescriptor descriptor) {
	...
	private void setUseDefaultJRE() {
		if (fDefaultDescriptor != null) {
			...
			fSpecificButton.setSelection(false);
			fCombo.setEnabled(false);
			// TODO fEnvironmentsCombo.setEnabled(false);
			firePropertyChange();
		}
	}
	...
	public IPath getPath() {
		...
		/*if (fEnvironmentsButton.getSelection()) {
			...
			if (index >= 0) {
				...
				return FASTRuntime.newJREContainerPath(env);
			}
			return null;
		}*/
		if (fSpecificButton.getSelection()) {
			int index = fCombo.getSelectionIndex();
			if (index >= 0) {
				IFASTInstall vm = (IFASTInstall) fVMs.get(index);
				return FASTRuntime.newJREContainerPath(vm);
			}
			return null;
		}
		return FASTRuntime.newDefaultJREContainerPath();
		...
		if (JavaRuntime.newDefaultJREContainerPath().equals(containerPath)) {
			...
			if (envId != null) {
				...
				if (install == null) {
					...
					if (installTypeId == null) {
						...
						if (installType == null) {
							...
							if (installName == null) {
								...
								setError(NLS.bind(FREMessages.JREsComboBlock_11, new String[]{installName, installType.getName()}));
							}
						}
					}
				} else {
					...
					if (location == null) {
						setError(FREMessages.JREsComboBlock_12); 
					} else if (!location.exists()) {
						setError(FREMessages.JREsComboBlock_13);
					}							
				}
			}
		}*/
	...
	private void setError(String message) {
		// TODO setStatus(new Status(IStatus.ERROR, FREIDebugUIPlugin.getUniqueIdentifier(),

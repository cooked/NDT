 ...
 */
public class StandardFASTRunner extends AbstractFASTRunner {
	
	/**
	 * The VM install instance
	 */
	protected IFASTInstall fVMInstance;

	/**
	 * Constructor
	 * @param vmInstance
	 */
	public StandardFASTRunner(IFASTInstall vmInstance) {
	 ...
	 */
	/* TODO protected String renderDebugTarget(String classToRun, int host) {
	...
	public static String renderProcessLabel(String[] commandLine) {
		...
		// TODO String timestamp= DateFormat.getDateTimeInstance(DateFormat.MEDIUM, DateFormat.MEDIUM).format(new Date(System.currentTimeMillis()));
		return format; // TODO MessageFormat.format(format, new String[] { commandLine[0], timestamp });
	}
	 ...
	 */	
	 protected File getWorkingDir(FASTRunnerConfiguration config) throws CoreException {
	...
	protected String getPluginIdentifier() {
		return null; // TODO LaunchingPlugin.getUniqueIdentifier();
	}
	...
	protected String constructProgramString(FASTRunnerConfiguration config) throws CoreException {
		...
		if (command == null) {
			File exe = null;
			if (fVMInstance instanceof StandardFAST) {
				exe = ((StandardFAST)fVMInstance).getJavaExecutable();
			} else {
		...
		// not found
		// TODO abort(MessageFormat.format(LaunchingMessages.StandardVMRunner_Specified_executable__0__does_not_exist_for__1__4, new String[]{command, fVMInstance.getName()}), null, IJavaLaunchConfigurationConstants.ERR_INTERNAL_ERROR); 
	 ...
	 */
	public void run(FASTRunnerConfiguration config, ILaunch launch, IProgressMonitor monitor) throws CoreException {
	...
	protected String[] prependJREPath(String[] env) {
		/* TODO if (Platform.OS_MACOSX.equals(Platform.getOS())) {
	 ...
	 * @param config
	 */
	protected void addBootClassPathArguments(List arguments, FASTRunnerConfiguration config) {
		String[] prependBootCP= null;
		String[] bootCP= null;
		String[] appendBootCP= null;
		Map map = config.getFASTSpecificAttributesMap();
		if (map != null) {
			/* TODO prependBootCP= (String[]) map.get(IJavaLaunchConfigurationConstants.ATTR_BOOTPATH_PREPEND);

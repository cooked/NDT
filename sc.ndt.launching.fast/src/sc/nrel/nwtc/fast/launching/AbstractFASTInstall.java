...
public abstract class AbstractFASTInstall implements IFASTInstall {

	private IFASTInstallType fType;
	private String fId;
	 ...
	 */
	public AbstractFASTInstall(IFASTInstallType type, String id) {
		/* TODO if (type == null)
	...
	public void setName(String name) {
		if (!name.equals(fName)) {
			PropertyChangeEvent event = new PropertyChangeEvent(this, IFASTInstallChangedListener.PROPERTY_NAME, fName, name);
			fName= name;
			if (fNotify) {
				FASTRuntime.fireVMChanged(event);
			}
		}
	}
	...
	public void setInstallLocation(File installLocation) {
		if (!installLocation.equals(fInstallLocation)) {
			PropertyChangeEvent event = new PropertyChangeEvent(this, IFASTInstallChangedListener.PROPERTY_INSTALL_LOCATION, fInstallLocation, installLocation);
	 ...
	 */
	public IFASTInstallType getFASTInstallType() {
		return fType;
	}
	...
	public boolean equals(Object object) {
		if (object instanceof IFASTInstall) {
			IFASTInstall vm = (IFASTInstall)object;
			return getFASTInstallType().equals(vm.getFASTInstallType()) &&
	 ...
	 */
	public int hashCode() {
		return getFASTInstallType().hashCode() + getId().hashCode();
	}
	...
	/*public Map evaluateSystemProperties(String[] properties, IProgressMonitor monitor) throws CoreException {
		...
		if (!cached) {      
			...
			if (file.exists()) {
				...
				FASTRunnerConfiguration config = new FASTRunnerConfiguration(mainType, new String[]{file.getAbsolutePath()});
				IFASTRunner runner = getFASTRunner(ILaunchManager.RUN_MODE);
				if (runner == null) {
					// TODO abort(LaunchingMessages.AbstractVMInstall_0, null, IJavaLaunchConfigurationConstants.ERR_INTERNAL_ERROR);
				}
	...
	private String getSystemPropertyKey(String property) {
		...
		buffer.append(getFASTInstallType().getId());
		buffer.append("."); //$NON-NLS-1$
		buffer.append(getId());
		buffer.append("."); //$NON-NLS-1$

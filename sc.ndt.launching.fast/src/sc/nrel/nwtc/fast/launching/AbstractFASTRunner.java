...
/**
 * Abstract implementation of a FAST runner.
 * <p>
 * Clients implementing FAST runners should subclass this class.
 * </p>
 * @see IFASTRunner
 * @since 1.0
 */
public abstract class AbstractFASTRunner implements IFASTRunner {
	...
	/**
	 * Returns the identifier of the plug-in this FAST runner 
	 * originated from.
	 * 
	...
	protected Map getDefaultProcessMap() {
		Map map = new HashMap();
		// TODO map.put(IProcess.ATTR_PROCESS_TYPE, IJavaLaunchConfigurationConstants.ID_JAVA_PROCESS_TYPE);
		return map;
	}
	...
	protected IProcess newProcess(ILaunch launch, Process p, String label, Map attributes) throws CoreException {
		...
		if (process == null) {
			p.destroy();
			// TODO abort(LaunchingMessages.AbstractVMRunner_0, null, IJavaLaunchConfigurationConstants.ERR_INTERNAL_ERROR); 
		}
		return process;
	}

	/**
	 * Combines and returns FAST arguments specified by the runner configuration,
	 * with those specified by the FAST install, if any.
	 * 
	 ...
	 * @since 3.0
	 */
	protected String[] combineVmArgs(FASTRunnerConfiguration configuration, IFASTInstall vmInstall) {
		String[] launchVMArgs = new String[0]; 	// TODO = configuration.getFASTArguments();
		String[] vmVMArgs = new String[0]; 		// TODO = vmInstall.getVMArguments();

...
import sc.ndt.editor.fast.fastfst.impl.vOutListImpl;


import static sc.ndt.editor.fast.ui.addon.coloring.FastfstHighlightingConfiguration.*;


public class FastfstSemanticHighlightingCalculator implements ISemanticHighlightingCalculator {
	...
	public void provideHighlightingFor(XtextResource resource, IHighlightedPositionAcceptor acceptor) {
		...
		while(it.hasNext()){
			...
			if( node instanceof RootNode )  {
				...
				if(esfValue!=null && !(se instanceof vOutListImpl)) {
				...
				} else if(esfValue!=null && se instanceof vOutListImpl) {
					lv = NodeModelUtils.findNodesForFeature(se, se.eClass().getEStructuralFeature(VALUE));
					
					if(	lv!=null 	&& 
							lv.size()>0 && 
							!(lv.get(0) instanceof CompositeNodeWithSyntaxError) ) {
							
							INode n = lv.get(0);
							rc = (RuleCallImpl)n.getGrammarElement();
							
							if( rc.getRule().getName().equalsIgnoreCase(OUTLIST))
									acceptor.addPosition( n.getOffset(), n.getLength(), OUTLIST);
					}
				}

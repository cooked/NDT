Input file for aeroacoustic code

340.46       	C0                 SPEED OF SOUND                     METERS/SEC
1.0       		ALPRAT             TIP LIFT CURVE SLOPE and(Default = 1.0)            ---
.TRUE.       	ROUND              LOGICAL INDICATING ROUNDED TIP     ---

1       IBLUNT             FLAG TO COMPUTE BLUNTNESS NOISE    ---
1       ILAM               FLAG TO COMPUTE LBL NOISE          ---
1       ITIP               FLAG TO COMPUTE TIP NOISE          ---
2       ITRIP              FLAG TO TRIP BOUNDARY LAYER        ---
1       ITURB              FLAG TO COMPUTE TBLTE NOISE        ---
1       IInflow            FLAG TO COMPUTE Turbulent Inflow NOISE        ---

4       NoiseOutSwitch     Switch to determine which time series of overall sound pressure level to output

-238.0  0.  84.0             (x,y,z) Observer location in tower-base coordinate system

Thickness    PSI 	
0.0025       12.5       
0.0025       12.5       
0.0025       12.5       
0.0025       12.5       
0.0025       12.5       
0.0025       12.5       
0.0025       12.5       
0.0025       12.5       
0.0025       12.5       
0.0025       12.5 
0.0025       12.5       
0.0025       12.5       
0.0025       12.5       
0.0025       12.5       
0.0025       12.5       
0.0025       12.5       
0.0025       12.5       
0.0025       12.5       
0.0025       12.5       
0.0025       12.5       

C       Thickness          SEGMENT TRAILING EDGE THICKNESS    METERS
C       PSI                SEGMENT BLUNTNESS ANGLE            DEGREES





